...
class BinaryTreeNode(object):
    ...
    def height(self):
        ...
        # If node has children on both left and right side
        if self.left != None and self.right != None:
            # Make reference to left and right child
            left_node = self.left
            right_node = self.right

            # get the total height of left side by recursively calling this function on every node on the left 
            left_height = left_node.height()
            # get the total height of right side by recursively calling this function on every node on the right
            right_height = right_node.height()

            # since we need the longest path, calculate if the left path was longner or the right
            max_height = max(left_height, right_height) 
            # Return one more than the greater of the left height and right height
            return max_height + 1

        # Check if we have child on the left, if so calculate its height
        elif self.left != None:
            node = self.left
        ...
        # if the node does not have children (we are at the end of the path), return 0
        else:
            return 0
...
class BinarySearchTree(object):
    ...
    def search(self, item):
        ...
        Best case running time: O(logn) 
        Worst case running time: O(h) where h is the height of the tree when it is unbalanced tree"""
        # Find a node with the given item, if any
        node = self._find_node_recursive(item, self.root)
    ...
    def _find_parent_node_iterative(self, item):
        ...
        Best case running time: O(logn) 
        Worst case running time: O(n) if unbalanced """
    ...
    def _traverse_in_order_recursive(self, node, visit):
        ...
        Memory usage: O(h) because we are creating a stack this will have at most the same number nodes as height of tree"""
        if node:
